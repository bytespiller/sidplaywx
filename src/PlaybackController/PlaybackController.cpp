...
PlaybackController::SwitchAudioDeviceResult PlaybackController::TrySwitchPlaybackConfiguration(const SyncedPlaybackConfig& newConfig)
{
    ...
    const bool needResetAudioOutput = (needResetSidDecoder && _preRender != nullptr) || // Also need to reset the prerender (in Instant Seeking mode) when the SID decoder gets reset since it'd hold an invalid reference to it then.
